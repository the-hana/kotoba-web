...
- `/study/:level/:dayId/quiz` ルートに 4択クイズページ (`QuizPage.tsx`) を新規追加
- 単語リスト画面のボタンを「フラッシュカード / クイズ」2列グリッドに変更
- オプション生成: 同じ DAY の単語を Fisher-Yates shuffle でランダム選択。追加 API 呼び出し不要
- 答え選択後 900ms で自動的に次問題へ進む設計。ユーザー操作を減らし学習テンポを維持するため

---

### フラッシュカードモードのバグ修正

- API エラー時に `words.length === 0` の空状態チェックより先にエラーを返すよう修正。以前は `useWords` がエラーを返しても「학습 가능한 단어가 없습니다」のみ表示されていた
- カード内側 `<div>` の `onClick` / `role` / `tabIndex` / `onKeyDown` / `aria-label` props のインデントを `className` と揃えて修正
